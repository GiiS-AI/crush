...
func TestLookupConfigs_BoundedByProject(t *testing.T) {
	...
	t.Run("does not pick up giis-code.json above non-git project", func(t *testing.T) {
		parent := t.TempDir()

		// giis-code.json above the project must not be adopted.
		require.NoError(t, os.WriteFile(
			filepath.Join(parent, "giis-code.json"),
			[]byte(`{}`),
			0o644,
		))
		...
		for _, p := range got {
			require.NotEqual(t, filepath.Join(parent, "giis-code.json"), p)
		}
	})

	t.Run("does not climb out of git worktree to find giis-code.json", func(t *testing.T) {
		...
		strayEval, err := filepath.EvalSymlinks(filepath.Join(parent, "giis-code.json"))
		require.NoError(t, err)
		for _, p := range got {
			...
			require.NotEqual(t, strayEval, pEval, "must not adopt parent giis-code.json")
		}
	})

	t.Run("picks up giis-code.json inside the project", func(t *testing.T) {
		project := t.TempDir()
		local := filepath.Join(project, "giis-code.json")
		...
		require.True(t, foundLocal, "expected project giis-code.json to be in lookup result: %v", got)
	})
...
func TestConfig_setDefaults(t *testing.T) {
	t.Run("sets default data directory", func(t *testing.T) {
		...
		require.Equal(t, filepath.Join(workingDir, ".giis-code"), cfg.Options.DataDirectory)
	...
	t.Run("does not adopt .giis-code from a parent project", func(t *testing.T) {
		parent := t.TempDir()

		// .giis-code in the parent: it should not be reused by the child
	...
	t.Run("does not climb out of git worktree to find .giis-code", func(t *testing.T) {
		if _, err := exec.LookPath("git"); err != nil {
			t.Skip("git not available")
		}
		...
		// Stray .giis-code above the worktree root.
		require.NoError(t, os.Mkdir(filepath.Join(parent, defaultDataDirectory), 0o755))
		...
		require.NoError(t, err)
		require.NotEqual(t, strayEval, gotEval, "must not adopt parent .giis-code")
...
func TestConfig_configureSelectedModels(t *testing.T) {
	t.Run("reload mode should not persist fallback defaults", func(t *testing.T) {
		dir := t.TempDir()
		globalPath := filepath.Join(dir, "giis-code.json")
