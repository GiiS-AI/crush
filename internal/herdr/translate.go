...
// Translate converts a pub/sub event (domain or proto) into a herdr Event.
// Returns nil for event types herdr doesn't care about. This is the single
// translation point for all integration modes.
...
// translateMessage is the shared message-mapping logic for both domain and
// proto message types.
...
// permNotificationSubscriber is the subset of the permission service needed
// by BridgeLocal to subscribe to permission notifications.
...
// BridgeSources groups the pub/sub sources that BridgeLocal subscribes to.
// Adding a new event type means adding a field here rather than growing the
// function signature.
...
// BridgeLocal subscribes to local pub/sub brokers and forwards translated
// events to the client. Used in TUI and local headless modes where the agent
// runs in-process. Cancelling ctx stops the bridge goroutines.
//
// The spawned goroutines are best-effort and may briefly outlive Client.Close.
// This is safe: HandleEvent is nil-safe, and the unixSender drops messages on
// a full buffer rather than blocking.
//
// Each goroutine uses a resilient subscription loop that re-subscribes if the
// channel closes unexpectedly, ensuring the bridge survives transient pub/sub
// broker resets.
...
// forward reads from a pub/sub channel and forwards translated events to the
// herdr client. If the channel closes (e.g. due to broker reset), it
