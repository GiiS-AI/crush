...
// are what non-interactive clients (e.g. `giis-code run`) rely on to
...
// through the SSE envelope. Without these correlators, `giis-code run`
...
// "agent failed" (returns non-zero from `giis-code run`) from "agent
