...
import (
	...
	_ "github.com/GiiS-AI/GiiS-Code/internal/swagger"
...
// socketDir returns the directory used for the GiiS-Code Unix socket.
...
// returned by [socketDir] and is named giis-code-<uid>.sock, falling
// back to giis-code.sock when the current uid cannot be determined. If
// the composed path would exceed [maxUnixSocketPathLen] bytes (the
// macOS sun_path limit), we fall back to /tmp/giis-code-<uid>.sock so
// the socket remains bindable.
func DefaultHost() string {
	sock := "giis-code.sock"
	usr, err := user.Current()
	if err == nil && usr.Uid != "" {
		sock = fmt.Sprintf("giis-code-%s.sock", usr.Uid)
	}
...
// Server represents a GiiS-Code server bound to a specific address.
type Server struct {
