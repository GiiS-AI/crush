...
func (q *QuestionToolRenderContext) RenderTool(sty *styles.Styles, width int, opts *ToolRenderOpts) string {
	...
	header := toolHeader(sty, opts.Status, "Question", cappedWidth, false, headerText)
	if opts.Compact {
		return header
	}
	...
	body := formatRenderedQuestionAnswers(sty, opts.Result.Content, cappedWidth-toolBodyLeftPaddingTotal)
	if body == "" {
		return header
	}
...
func formatRenderedQuestionAnswers(sty *styles.Styles, content string, width int) string {
	if content == "" {
		return ""
	}
	...
	for _, b := range blocks {
		...
		answer := styleQuestionAnswer(sty, b.answer)
...
func styleQuestionAnswer(sty *styles.Styles, answer string) string {
	...
	for _, line := range lines {
		...
		styled = append(styled, styleQuestionAnswerLine(sty, line))
	}
	return strings.Join(styled, sty.Tool.TodoStatusNote.Render(", "))
}

func styleQuestionAnswerLine(sty *styles.Styles, answer string) string {
	switch {
