...
// event into a closure nobody reads, and subscribers (`giis-code run`)
...
// publish a RunComplete for its RunID, hanging a `giis-code run` caller that
...
// than internal bookkeeping, which is the contract a `giis-code run` caller
...
// bookkeeping slice, not the published event a `giis-code run` caller awaits.
