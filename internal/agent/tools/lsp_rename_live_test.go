...
func TestLiveRenameToolAppliesEdit(t *testing.T) {
	...
	require.NoError(t, os.WriteFile(filepath.Join(workspace, "go.mod"), []byte("module example.com/live-rename-tool\n\ngo 1.23\n"), 0o644))
...
`
	...
	// No session ID in ctx, and permissions/files/filetracker are all nil:
	// the tool's own logic gates every one of those on sessionID != "", so
	// this exercises exactly the resolveSymbol + Rename + ApplyWorkspaceEdit
	// path with nothing stubbed out.
	tool := NewRenameTool(manager, nil, nil, nil)

	input, err := json.Marshal(RenameParams{
		Symbol:  "helper",
	...
	resp, err := tool.Run(ctx, fantasy.ToolCall{ID: "test-call", Name: RenameToolName, Input: string(input)})
	require.NoError(t, err)
	...
	require.Contains(t, content, "func renamedHelper() string {")
	require.Contains(t, content, "return renamedHelper()")
	require.NotContains(t, content, "func helper() string {")
	require.NotContains(t, content, "return helper()")

	// The unrelated symbol must be untouched.
