...
// use resolveSymbol; callers that need to iterate candidates use this directly.
func resolveSymbolResults(ctx context.Context, lspManager *lsp.Manager, symbol, workingDir string) ([]*resolvedSymbol, error) {
	...
	searchRoot := cmp.Or(workingDir, ".")
	if absRoot, err := filepath.Abs(searchRoot); err == nil {
		searchRoot = absRoot
	}

	lspManager.Start(ctx, searchRoot)

	pattern := `\b` + regexp.QuoteMeta(symbol) + `\b`
	matches, _, err := searchFiles(ctx, pattern, searchRoot, "", 100)
	if err != nil {
	...
	results := make([]*resolvedSymbol, 0, len(matches))
