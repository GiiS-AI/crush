	...
	return fantasy.NewAgentTool(
		...
		func(ctx context.Context, params ReferencesParams, call fantasy.ToolCall) (fantasy.ToolResponse, error) {
			...
			for _, result := range results {
				locations, err := result.client.FindReferences(ctx, result.path, result.line, result.char, true)
				if err != nil {
					if isNoIdentifierError(err) {
						continue
					}
					slog.Error("Failed to find references", "error", err, "symbol", params.Symbol, "path", result.path, "line", result.line, "char", result.char)
