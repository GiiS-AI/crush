...
type RunOptions struct {
	...
	// the GiiS-Code process cwd — hooks and the bash tool have different
...
// standard GiiS-Code middleware chain (builtins, script dispatch, block list)
...
// when sourcing framework files can send SIGINT/SIGTERM to GiiS-Code's process
...
// regardless of the caller's environment because GiiS-Code shells are never
...
//  1. builtins first (so GiiS-Code's in-process jq wins over any PATH binary);
...
// builtinHandler returns middleware that dispatches recognized GiiS-Code
