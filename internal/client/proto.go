...
// AnswerQuestionBatch submits answers for a question batch on a workspace.
...
// CancelQuestionBatch cancels the pending question batch on a workspace.
func (c *Client) CancelQuestionBatch(ctx context.Context, id string, req proto.QuestionCancel) (bool, error) {
	rsp, err := c.post(ctx, fmt.Sprintf("/workspaces/%s/questions/cancel", id), nil, jsonBody(req), http.Header{"Content-Type": []string{"application/json"}})
	if err != nil {
