...
func TestToPromptXMLBuiltinType(t *testing.T) {
	...
	skills := []*Skill{
		{Name: "builtin-skill", Description: "A builtin.", SkillFilePath: "giis-code://skills/builtin-skill/SKILL.md", Builtin: true},
		{Name: "user-skill", Description: "A user skill.", SkillFilePath: "/home/user/.config/giis-code/skills/user-skill/SKILL.md"},
	}
	xml := ToPromptXML(skills)
...
func TestDiscoverBuiltin(t *testing.T) {
	...
	for _, s := range discovered {
		if s.Name == "giis-code-config" {
			found = true
			require.True(t, strings.HasPrefix(s.SkillFilePath, BuiltinPrefix))
			require.True(t, strings.HasPrefix(s.Path, BuiltinPrefix))
			require.Equal(t, "giis-code://skills/giis-code-config/SKILL.md", s.SkillFilePath)
			require.Equal(t, "giis-code://skills/giis-code-config", s.Path)
			require.NotEmpty(t, s.Description)
			require.NotEmpty(t, s.Instructions)
			require.True(t, s.Builtin)
		}
	}
	require.True(t, found, "giis-code-config builtin skill not found")

	var foundJQ bool
	for _, s := range discovered {
		if s.Name == "jq" {
			foundJQ = true
			require.Equal(t, "giis-code://skills/jq/SKILL.md", s.SkillFilePath)
			require.Equal(t, "giis-code://skills/jq", s.Path)
			require.NotEmpty(t, s.Description)
			require.NotEmpty(t, s.Instructions)
			require.True(t, s.Builtin)
		}
	...
	var foundHooks bool
	for _, s := range discovered {
		if s.Name == "giis-code-hooks" {
			foundHooks = true
			require.Equal(t, "giis-code://skills/giis-code-hooks/SKILL.md", s.SkillFilePath)
			require.Equal(t, "giis-code://skills/giis-code-hooks", s.Path)
			require.NotEmpty(t, s.Description)
			require.NotEmpty(t, s.Instructions)
			require.True(t, s.Builtin)
		}
	}
	require.True(t, foundHooks, "giis-code-hooks builtin skill not found")
}
...
func TestDeduplicate(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			input:    []*Skill{{Name: "giis-code-config", Path: "giis-code://skills/giis-code-config"}, {Name: "giis-code-config", Path: "/user/giis-code-config"}},
			wantLen:  1,
			wantName: "giis-code-config",
			wantPath: "/user/giis-code-config",
		},
		{
			name:    "empty",
			input:   nil,
