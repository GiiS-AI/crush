...
func TestTracker_BuiltinSkillTracking(t *testing.T) {
	...
	// Simulate active skills including a builtin skill (giis-code-config).
	activeSkills := []*Skill{
		{Name: "giis-code-config", Description: "GiiS-Code config", Builtin: true},
		{Name: "go-doc", Description: "Go docs", Builtin: false},
	}
	tracker := NewTracker(activeSkills)

	// Initially not loaded.
	require.False(t, tracker.IsLoaded("giis-code-config"))
	require.False(t, tracker.IsLoaded("go-doc"))

	// Mark builtin skill as loaded (simulating read via giis-code://...).
	tracker.MarkLoaded("giis-code-config")
	require.True(t, tracker.IsLoaded("giis-code-config"))
