...
// with reason=tool_use and used to terminate `giis-code run` early via
...
// subscribers (e.g. a TUI watching workspace events while `giis-code
...
// non-nil error from `giis-code run` so shells and CI catch it via
// exit status.
...
// run (e.g. Ctrl+C while `giis-code run` waits) exits cleanly rather
...
// queue scenario: `giis-code run --continue` attaches to a session
