...
The prompt can be provided as arguments or piped from stdin.`,
	Example: `
# Run a simple prompt
giis-code run "Guess my 5 favorite Pokémon"

# Pipe input from stdin
curl https://charm.land | giis-code run "Summarize this website"

# Read from a file
giis-code run "What is this code doing?" <<< prrr.go

# Redirect output to a file
giis-code run "Generate a hot README for this project" > MY_HOT_README.md

# Run in quiet mode (hide the spinner)
giis-code run --quiet "Generate a README for this project"

# Run in verbose mode (show logs)
giis-code run --verbose "Generate a README for this project"

# Continue a previous session
giis-code run --session {session-id} "Follow up on your last response"

# Continue the most recent session
giis-code run --continue "Follow up on your last response"
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if useClientServer() {
			...
			if !ws.Config.IsConfigured() {
				return fmt.Errorf("no providers configured - please run 'giis-code' to set up a provider interactively")
			}
...
// streaming SSE events into a final, complete stdout for `giis-code run`.
...
func (s *runStream) handle(ev any, stopSpinner func()) (done bool, err error) {
	...
	case pubsub.Event[proto.RunComplete]:
		...
		// source of the regression where `giis-code run` exited
...
// prefix returned by giis-code session list.
